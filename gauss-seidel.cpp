...
void gaussSeidelMethod(double coeffs[100][100], double *b, int m, int n, bool verbose = false) {
    ...
    for(int c =0;c<10;c++) {
        ...
        for(i=0;i<n;i++) {
            ...
            for(j=0;j<m;j++) {
                if(j != i) {
                    res -= coeffs[i][j]*soln[j];
                    if(verbose) cout << "-"<<coeffs[i][j]<< "*" << soln[j];
                }
            }
...
int main() {
    ...
    gaussSeidelMethod(coeffs, b, m, n,true);
